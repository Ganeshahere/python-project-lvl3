...
with open(os.path.join(sys.path[0], 'fixtures/page/with_local_links.html'), 'r') as file:
...
def test_download():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        with requests_mock.Mocker() as m:
            ...
            m.get(RESOURCES_LINK, text=RECOURCES_LINK)
            file_path = download(URL, tmpdirname)
            with open(file_path, 'r') as file:
                page = file.read()
            assert page == expected_page
            assert len(os.listdir(tmpdirname)) == 2
            assert len(os.listdir(os.path.join(tmpdirname, 'test-com_file'))) == 1
